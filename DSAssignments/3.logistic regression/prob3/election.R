data = read.csv("C:/Users/Rajesh/Downloads/data_assignments/mod9/election_data.csv")
head(data)
data <- data[-1,]
View(data)
attach(data)
...
plot(data$Result)
plot(data$Amount.Spent)
plot(data$Year)
plot(data$Popularity.Rank)
plot(data)
...
#preparing my model
colnames(data)
model <- glm(Result~Amount.Spent+Popularity.Rank,data=data, family='binomial')
summary(model)
#null deviance is 13.4602 (Y=B0 aka average)
#residual devaince is 3.8177 (Y=B0+ other variabels (b1x1,b2x2,etc))
#residual dev is less than null as expected, and the difference is large thus the variables are quality variables
# AIC value is 9.8177, model with least AIC value is best
...
table(data$Result,data$pred_values)
